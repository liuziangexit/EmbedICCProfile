...
static constexpr unsigned char icc_profile_identify[]{
    0x49, 0x43, 0x43, 0x5f, 0x50, 0x52, 0x4f,
    0x46, 0x49, 0x4c, 0x45, 0x00, 0x01, 0x01};
static constexpr int soi_and_app0_size_without_thumbnail =
    sizeof(soi) + sizeof(app0_marker) + 16;
...
inline int16_t to_int_16(const unsigned char *tail, int index) {
  if (is_little_endian)
    return static_cast<int16_t>(static_cast<uint16_t>(tail[index + 1] << 8) +
                                static_cast<unsigned char>(tail[index]));
  else
    return static_cast<int16_t>(static_cast<uint16_t>(tail[index] << 8) +
                                static_cast<unsigned char>(tail[index + 1]));
}
...
//计算图像置入色彩文件后的大小
inline uint32_t get_expected_file_size(uint32_t image_size,
                                       uint32_t icc_profile_size) {
  return image_size + sizeof(app2_marker) + segment_length_size +
         sizeof(icc_profile_identify) + icc_profile_size;
}

//置入色彩文件到图像
// image是图片缓冲区，image_size是图片实际大小，image_reserve_size
// 是图片缓冲区大小，icc_profile是色彩文件缓冲区，icc_profile_size是色彩文件大小，output_size指向的对象将会在函数执行后得到嵌入色彩文件后图片的大小
//成功返回true，失败返回false
bool EmbedICCProfile(unsigned char *const image, int32_t image_size,
                     int32_t image_reserve_size,
                     const unsigned char *const icc_profile,
                     int32_t icc_profile_size, int32_t *const output_size) {
  ...
  std::size_t expected_file_size =
      image_size + sizeof(app2_marker) + segment_length_size +
      sizeof(icc_profile_identify) + icc_profile_size;
  ...
  if (is_little_endian)
    std::reverse(app0_header_size_pun,
                 app0_header_size_pun + segment_length_size);

  int16_t app0_header_size = to_int_16(app0_header_size_pun, 0);
  uint32_t app2_segment_size =
      icc_profile_size + sizeof(icc_profile_identify) + segment_length_size;

  if (is_little_endian) {
    app2_segment_size_buffer[0] =
        *(reinterpret_cast<unsigned char *>(&app2_segment_size) + 1);
    app2_segment_size_buffer[1] =
        *(reinterpret_cast<unsigned char *>(&app2_segment_size));
  } else {
    memcpy(app2_segment_size_buffer,
           reinterpret_cast<unsigned char *>(&app2_segment_size), 2);
  }

  std::unique_ptr<unsigned char[]> jpg_image_without_header(
      new unsigned char[image_size]);
  memcpy(jpg_image_without_header.get(), image, image_size);

  unsigned char *ptr2 =
      image + sizeof(soi) + sizeof(app0_marker) + app0_header_size;
  ...
  memcpy(ptr2,
         jpg_image_without_header.get() + sizeof(soi) + sizeof(app0_marker) +
             app0_header_size,
         image_size - (app0_header_size + sizeof(soi) + sizeof(app0_marker)));
...
int main(int argc, char **argv) {
  {
    ...
    if (fread(jpg.get(), sizeof(unsigned char), reserve_size, fp.get()) !=
        jpg_size) {
    ...
    if (fread(icc.get(), sizeof(unsigned char), icc_size, fp.get()) !=
        icc_size) {
    ...
    int outsize = 0;
    if (!EmbedICCProfile(jpg.get(), jpg_size, reserve_size, icc.get(), icc_size,
                         &outsize)) {
    ...
    if (fwrite(jpg.get(), sizeof(unsigned char), outsize, fp.get()) !=
        outsize) {
      std::cout << "write file failed\n";
      return 0;
    }
